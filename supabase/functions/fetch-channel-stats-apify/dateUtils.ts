...
export function formatDate(dateString: string): string | null {
  ...
  try {
    ...
    const joinedMatch = dateString.match(/Joined\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{1,2}),?\s+(\d{4})/i);
    if (joinedMatch) {
      ...
      const month = monthMap[joinedMatch[1].toLowerCase()];
    ...
    const standardMatch = dateString.match(/(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{1,2}),?\s+(\d{4})/i);
    if (standardMatch) {
      const monthMap: Record<string, string> = {
        'jan': '01', 'feb': '02', 'mar': '03', 'apr': '04', 'may': '05', 'jun': '06',
        'jul': '07', 'aug': '08', 'sep': '09', 'oct': '10', 'nov': '11', 'dec': '12'
      ...
      const month = monthMap[standardMatch[1].toLowerCase()];
